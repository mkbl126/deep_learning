...
def build_style_transfer_graph(model_name="VGG16") :
    K.clear_session()
    ...
    loss_content = get_content_loss(model, content_layer_names, src1_array)
    loss_style = get_style_loss(model, style_layer_names, src2_array)
    ...
    optimize_image = K.function([combination_image], [loss_content, loss_style, loss_denoise, loss, grad])
...
folder_dest = r"C:\DL\images\style_transfer"
    ...
    if os.path.isfile(src1_image_path) :
        ...
        for file_src2 in files_src2 :
            ...
            if os.path.isfile(src2_image_path) :
                ...
                if os.path.exists(combination_image_path)==False :
                    optimize_image = build_style_transfer_graph("VGG16")
