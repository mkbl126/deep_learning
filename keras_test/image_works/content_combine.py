...
def build_content_combine_graph(model_name="VGG16") :
    K.clear_session()
    ...
    loss_content1 = get_content_loss(model, content_layer_names, src1_array)
    loss_content2 = get_content_loss(model, content_layer_names, src2_array)
    loss = weight_content1*loss_content1 + weight_content2*loss_content2
    grad = K.gradients(loss, combination_image)[0]
    optimize_image = K.function([combination_image], [loss_content1, loss_content2, loss, grad])
...
weight_content1 = 15.0
weight_content2 = 7.0

step_size = 10
num_iterations = 100

folder_src1 = r"C:\DL\images\contents"
folder_src2 = r"C:\DL\images\styles"
folder_dest = r"C:\DL\images\content_combine"
    ...
    if os.path.isfile(src1_image_path) :
        ...
        for file_src2 in files_src2 :
            ...
            if os.path.isfile(src2_image_path) :
                ...
                optimize_image = build_content_combine_graph("VGG16")
                ...
                for i in range(num_iterations) :
                    ...
                    loss_content1, loss_content2, loss, grad = optimize_image([x])
                    step_size_scaled = step_size / (np.std(grad) + 1e-8)
                    x -= grad*step_size_scaled
                    x = np.clip(x, 0.0, 255.0)
                    end_time = time.time()
                    # print("Iteration %02d in %ds. L_content1 %f, L_content1 %f, Loss %f" %
                    #       (i, end_time-start_time, loss_content1, loss_content2, loss))
